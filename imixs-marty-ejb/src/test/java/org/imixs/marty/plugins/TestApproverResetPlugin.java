...
public class TestApproverResetPlugin {
	...
	public void testSimpleApproval() throws PluginException, ModelException {
		...
		nameList.add("eddy");
		workitem.replaceItemValue("namProcessManager", nameList);
		...
		Assert.assertEquals(3, workitem.getItemValue("namProcessManagerApprovers").size());
		Assert.assertEquals(0, workitem.getItemValue("namProcessManagerApprovedBy").size());
		
		
		// 200.20 eddy
		workitem.event(20);
		workitem = workflowMockEnvironment.getWorkflowService().processWorkItem(workitem);
		Assert.assertEquals(2, workitem.getItemValue("namProcessManagerApprovers").size());
		Assert.assertEquals(1, workitem.getItemValue("namProcessManagerApprovedBy").size());
		...
		workitem.event(20);
		workitem = workflowMockEnvironment.getWorkflowService().processWorkItem(workitem);
		Assert.assertEquals(1, workitem.getItemValue("namProcessManagerApprovers").size());
		Assert.assertEquals(2, workitem.getItemValue("namProcessManagerApprovedBy").size());
		...
		workitem.event(20);
		workitem = workflowMockEnvironment.getWorkflowService().processWorkItem(workitem);
		Assert.assertEquals(0, workitem.getItemValue("namProcessManagerApprovers").size());
		Assert.assertEquals(3, workitem.getItemValue("namProcessManagerApprovedBy").size());
